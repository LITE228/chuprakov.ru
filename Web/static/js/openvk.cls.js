...
document.addEventListener("DOMContentLoaded", function() { //BEGIN
	...
	u("#_avatarDelete").on("click", function(e) {
        ...
        MessageBox(tr('warning'), tr('delete_photo'), [
            tr('delete'),
            tr('close')
        ], [
            (function() {
...
async function ajaxError(error, errorDesc, type)
{
    let ttp = type == "succ" ? "msg_succ" : "msg_err";
    if(document.querySelector("."+ttp) == null) {
        let err = document.querySelector(".page_content")
        ...
        document.querySelector("."+ttp).parentNode.removeChild(document.querySelector("."+ttp))
    }
}
